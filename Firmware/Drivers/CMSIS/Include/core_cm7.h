...
typedef struct {
    ...
    __IOM uint8_t IP
        [240U]; /*!< Offset: 0x300 (R/W)  Interrupt Priority Register (8Bit wide) */
...
typedef struct {
    ...
    __IM uint32_t
        ID_AFR; /*!< Offset: 0x04C (R/ )  Auxiliary Feature Register */
    __IM uint32_t
...
__STATIC_INLINE uint32_t __NVIC_GetPriority(IRQn_Type IRQn)
{
    ...
    if ((int32_t)(IRQn) >= 0) {
        return (
            ((uint32_t)NVIC->IP[((uint32_t)IRQn)] >> (8U - __NVIC_PRIO_BITS)));
    } else {
...
#define __SCB_DCACHE_LINE_SIZE                                                 \
    32U /*!< Cortex-M7 cache line size is fixed to 32 bytes (8 words). See also register SCB_CCSIDR */
