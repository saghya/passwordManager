...
HAL_StatusTypeDef HAL_RCC_DeInit(void)
{
    ...
    while (READ_BIT(RCC->CR, RCC_CR_PLLRDY | RCC_CR_PLLSAI1RDY |
                                 RCC_CR_PLLSAI2RDY) != 0U)
...
HAL_StatusTypeDef HAL_RCC_OscConfig(RCC_OscInitTypeDef *RCC_OscInitStruct)
{
    ...
    if (((RCC_OscInitStruct->OscillatorType) & RCC_OSCILLATORTYPE_MSI) ==
        ...
        assert_param(IS_RCC_MSICALIBRATION_VALUE(
            RCC_OscInitStruct->MSICalibrationValue));
        assert_param(IS_RCC_MSI_CLOCK_RANGE(RCC_OscInitStruct->MSIClockRange));

        /* Check if MSI is used as system clock or as PLL source when PLL is
        ...
        if ((sysclk_source == RCC_CFGR_SWS_MSI) ||
             ...
             */
                ...
                /* To correctly read data from FLASH memory, the number of wait
                   states (LATENCY)
                   must be correctly programmed according to the frequency of the CPU clock
                   (HCLK) and the supply voltage of the device. */
                if (RCC_OscInitStruct->MSIClockRange >
                    ...
                    /* Adjusts the Multiple Speed oscillator (MSI) calibration
                     * value.*/
    ...
    if (((RCC_OscInitStruct->OscillatorType) & RCC_OSCILLATORTYPE_HSI) ==
        ...
        assert_param(IS_RCC_HSI_CALIBRATION_VALUE(
    ...
    if (((RCC_OscInitStruct->OscillatorType) & RCC_OSCILLATORTYPE_LSI) ==
        ...
        if (RCC_OscInitStruct->LSIState != RCC_LSI_OFF) {
            ...
            if (RCC_OscInitStruct->LSIDiv != (csr_temp & RCC_CSR_LSIPREDIV)) {
                ...
                MODIFY_REG(RCC->CSR, RCC_CSR_LSIPREDIV,
    ...
    if (((RCC_OscInitStruct->OscillatorType) & RCC_OSCILLATORTYPE_LSE) ==
        ...
        if ((RCC_OscInitStruct->LSEState & RCC_BDCR_LSEON) != 0U) {
            /* Set LSESYSDIS bit according to LSE propagation option (enabled or
             * disabled) */
            MODIFY_REG(RCC->BDCR, RCC_BDCR_LSESYSDIS,
                       (RCC_OscInitStruct->LSEState & RCC_BDCR_LSESYSDIS));
        ...
        if (RCC_OscInitStruct->PLL.PLLState == RCC_PLL_ON) {
            ...
            if ((READ_BIT(pll_config, RCC_PLLCFGR_PLLSRC) !=
                ...
                (READ_BIT(pll_config, RCC_PLLCFGR_PLLP) !=
                 ((RCC_OscInitStruct->PLL.PLLP == RCC_PLLP_DIV7) ? 0U : 1U)) ||
#endif
#endif
                ...
                if (sysclk_source != RCC_CFGR_SWS_PLL) {
                    ...
                    /* Not possible if the source is shared by other enabled
                     * PLLSAIx */
            ...
            if (sysclk_source != RCC_CFGR_SWS_PLL) {
                ...
                RCC->PLLCFGR &= ~(RCC_PLLCFGR_PLLSRC | RCC_PLL_SYSCLK |
                                  RCC_PLL_48M1CLK | RCC_PLL_SAI3CLK);
#elif defined(RCC_PLLSAI1_SUPPORT)
                RCC->PLLCFGR &= ~(RCC_PLLCFGR_PLLSRC | RCC_PLL_SYSCLK |
                                  RCC_PLL_48M1CLK | RCC_PLL_SAI2CLK);
#else
                RCC->PLLCFGR &=
                    ~(RCC_PLLCFGR_PLLSRC | RCC_PLL_SYSCLK | RCC_PLL_48M1CLK);
#endif /* RCC_PLLSAI2_SUPPORT */
            } else {
                /* PLL is already used as System core clock */
                return HAL_ERROR;
                                      ...
                                      uint32_t            FLatency)
{
    ...
    /* Apply higher HCLK prescaler request here to ensure CPU clock is not of of
     * spec when SYSCLK is increased */
    ...
    defined(STM32L4R7xx) || defined(STM32L4R9xx) || defined(STM32L4S5xx) ||    \
    defined(STM32L4S7xx) || defined(STM32L4S9xx)
            /* Undershoot management when selection PLL as SYSCLK source and
             * frequency above 80Mhz */
            /* Compute target PLL output frequency */
            if (RCC_GetSysClockFreqFromPLLSource() > 80000000U) {
                /* If lowest HCLK prescaler, apply intermediate step with HCLK
                 * prescaler 2 necessary before to go over 80Mhz */
    ...
    defined(STM32L4R7xx) || defined(STM32L4R9xx) || defined(STM32L4S5xx) ||    \
    defined(STM32L4S7xx) || defined(STM32L4S9xx)
            /* Overshoot management when going down from PLL as SYSCLK source
             * and frequency above 80Mhz */
            if (HAL_RCC_GetSysClockFreq() > 80000000U) {
                /* If lowest HCLK prescaler, apply intermediate step with HCLK
                 * prescaler 2 necessary before to go under 80Mhz */
                if (READ_BIT(RCC->CFGR, RCC_CFGR_HPRE) == RCC_SYSCLK_DIV1) {
                    MODIFY_REG(RCC->CFGR, RCC_CFGR_HPRE, RCC_SYSCLK_DIV2);
                    hpre = RCC_SYSCLK_DIV2;
                }
    ...
    defined(STM32L4R7xx) || defined(STM32L4R9xx) || defined(STM32L4S5xx) ||    \
    defined(STM32L4S7xx) || defined(STM32L4S9xx)
    /* Is intermediate HCLK prescaler 2 applied internally, resume with HCLK
     * prescaler 1 */
    ...
    /* Apply lower HCLK prescaler request here to ensure CPU clock is not of of
     * spec when SYSCLK is set */
    if (((RCC_ClkInitStruct->ClockType) & RCC_CLOCKTYPE_HCLK) ==
        RCC_CLOCKTYPE_HCLK) {
        if (RCC_ClkInitStruct->AHBCLKDivider <
            READ_BIT(RCC->CFGR, RCC_CFGR_HPRE)) {
    ...
    /* Allow decreasing of the number of wait states (because of lower CPU
     * frequency expected) */
    if (FLatency < __HAL_FLASH_GET_LATENCY()) {
        /* Program the new number of wait states to the LATENCY bits in the
         * FLASH_ACR register */
...
uint32_t HAL_RCC_GetSysClockFreq(void)
{
    ...
    if ((sysclk_source == RCC_CFGR_SWS_MSI) ||
        ...
        if (READ_BIT(RCC->CR, RCC_CR_MSIRGSEL) ==
            ...
            msirange =
                READ_BIT(RCC->CSR, RCC_CSR_MSISRANGE) >> RCC_CSR_MSISRANGE_Pos;
    ...
    if (sysclk_source == RCC_CFGR_SWS_PLL) {
        ...
        */
        ...
        pllm =
            ...
            1U;
        pllvco = (pllvco * (READ_BIT(RCC->PLLCFGR, RCC_PLLCFGR_PLLN) >>
...
void HAL_RCC_GetOscConfig(RCC_OscInitTypeDef *RCC_OscInitStruct)
{
    ...
    RCC_OscInitStruct->MSICalibrationValue =
        READ_BIT(RCC->ICSCR, RCC_ICSCR_MSITRIM) >> RCC_ICSCR_MSITRIM_Pos;
    ...
    RCC_OscInitStruct->PLL.PLLQ =
        (((READ_BIT(RCC->PLLCFGR, RCC_PLLCFGR_PLLQ) >> RCC_PLLCFGR_PLLQ_Pos) +
          1U)
         << 1U);
    RCC_OscInitStruct->PLL.PLLR =
        (((READ_BIT(RCC->PLLCFGR, RCC_PLLCFGR_PLLR) >> RCC_PLLCFGR_PLLR_Pos) +
          1U)
         << 1U);
#if defined(RCC_PLLP_SUPPORT)
#if defined(RCC_PLLP_DIV_2_31_SUPPORT)
...
static uint32_t RCC_GetSysClockFreqFromPLLSource(void)
{
    uint32_t msirange, pllvco, pllsource, pllr, pllm,
        sysclockfreq; /* no init needed */
