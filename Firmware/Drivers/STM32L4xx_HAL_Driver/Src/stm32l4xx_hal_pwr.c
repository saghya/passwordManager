 ...
 */
#define PVD_MODE_IT                                                            \
    ((uint32_t)0x00010000) /*!< Mask for interruption yielded by PVD threshold crossing */
#define PVD_MODE_EVT                                                           \
 ...
 *        In order to set or modify the RTC clock, the backup domain access must
 * be disabled.
 ...
 *        in Low-power Run mode before calling HAL_PWR_EnterSLEEPMode() with
 ...
 *        Run mode, the user must resort to HAL_PWREx_DisableLowPowerRunMode()
 * API.
 ...
 *        on devices where only "Stop mode" is mentioned with main or low power
 * regulator ON.
 ...
 *         By keeping the internal regulator ON during Stop mode (Stop 0), the
 ...
 * HAL_PWREx_DisableGPIOPullUp() and HAL_PWREx_DisableGPIOPullDown() disable the
 *        same.
 *        These states are effective in Standby mode only if APC bit is set
 * through HAL_PWREx_EnablePullUpPullDownConfig() API.
 * @retval None
 */
 ...
 *       Setting this bit is useful when the processor is expected to run only
 * on interruptions handling.
 * @retval None
 */
