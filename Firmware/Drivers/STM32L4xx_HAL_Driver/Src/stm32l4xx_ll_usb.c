                                        ...
                                        uint32_t hclk, uint8_t speed)
{
    ...
    /* The USBTRD is configured according to the tables below, depending on AHB
    frequency
                                         ...
                                         USB_OTG_EPTypeDef     *ep)
{
    ...
    if (ep->is_in == 1U) {
        ...
        USBx_DEVICE->DAINTMSK &= ~(USB_OTG_DAINTMSK_IEPM &
                                   (uint32_t)(1UL << (ep->num & EP_ADDR_MSK)));
        USBx_INEP(epnum)->DIEPCTL &=
            ~(USB_OTG_DIEPCTL_USBAEP | USB_OTG_DIEPCTL_MPSIZ |
        ...
        USBx_DEVICE->DAINTMSK &=
            ~(USB_OTG_DAINTMSK_OEPM &
              ((uint32_t)(1UL << (ep->num & EP_ADDR_MSK)) << 16));
        USBx_OUTEP(epnum)->DOEPCTL &=
            ~(USB_OTG_DOEPCTL_USBAEP | USB_OTG_DOEPCTL_MPSIZ |
                                  ...
                                  USB_OTG_EPTypeDef     *ep)
{
    ...
    if (ep->is_in == 1U) {
        ...
        if (ep->xfer_len == 0U) {
            ...
            USBx_INEP(epnum)->DIEPTSIZ |=
                (USB_OTG_DIEPTSIZ_XFRSIZ & ep->xfer_len);

            if (ep->type == EP_TYPE_ISOC) {
                USBx_INEP(epnum)->DIEPTSIZ &= ~(USB_OTG_DIEPTSIZ_MULCNT);
                USBx_INEP(epnum)->DIEPTSIZ |=
                    (USB_OTG_DIEPTSIZ_MULCNT & (1U << 29));
            }
        }
    ...
    } else /* OUT endpoint */
    {
        ...
        if (ep->xfer_len == 0U) {
            ...
            pktcnt =
                (uint16_t)((ep->xfer_len + ep->maxpacket - 1U) / ep->maxpacket);
            ep->xfer_size = ep->maxpacket * pktcnt;

            USBx_OUTEP(epnum)->DOEPTSIZ |=
                USB_OTG_DOEPTSIZ_PKTCNT & ((uint32_t)pktcnt << 19);
            USBx_OUTEP(epnum)->DOEPTSIZ |=
                                   ...
                                   USB_OTG_EPTypeDef     *ep)
{
    ...
    } else /* OUT endpoint */
    {
        ...
        /* Store transfer size, for EP0 this is equal to endpoint max packet
         * size */
                                   ...
                                   USB_OTG_EPTypeDef     *ep)
{
    ...
    if (ep->is_in == 1U) {
        ...
        if ((ep->type == EP_TYPE_INTR) || (ep->type == EP_TYPE_BULK)) {
            USBx_INEP(epnum)->DIEPCTL |=
                USB_OTG_DIEPCTL_SD0PID_SEVNFRM; /* DATA0 */
        }
    } else {
        USBx_OUTEP(epnum)->DOEPCTL &= ~USB_OTG_DOEPCTL_STALL;
        if ((ep->type == EP_TYPE_INTR) || (ep->type == EP_TYPE_BULK)) {
                               ...
                               USB_OTG_CfgTypeDef     cfg)
{
    ...
    /* set Rx FIFO size */
    USBx->GRXFSIZ = 0x80U;
    USBx->DIEPTXF0_HNPTXFSIZ =
        (uint32_t)(((0x60U << 16) & USB_OTG_NPTXFD) | 0x80U);
    USBx->HPTXFSIZ =
        (uint32_t)(((0x40U << 16) & USB_OTG_HPTXFSIZ_PTXFD) | 0xE0U);
    /* Enable the common interrupts */
    USBx->GINTMSK |= USB_OTG_GINTMSK_RXFLVLM;
                              ...
                              uint8_t ep_type, uint16_t mps)
{
    ...
    USBx_HC((uint32_t)ch_num)->HCCHAR =
        (((uint32_t)dev_address << 22) & USB_OTG_HCCHAR_DAD) |
                                   ...
                                   USB_OTG_HCTypeDef     *hc)
{
    ...
    if (hc->xfer_len > 0U) {
        num_packets =
            (uint16_t)((hc->xfer_len + hc->max_packet - 1U) / hc->max_packet);
    ...
    USBx_HC(ch_num)->HCTSIZ =
        (hc->XferSize & USB_OTG_HCTSIZ_XFRSIZ) |
    ...
    if ((hc->ep_is_in == 0U) && (hc->xfer_len > 0U)) {
        switch (hc->ep_type) {
            ...
            case EP_TYPE_ISOC:
                ...
                /* check if there is enough space in FIFO space */
                if (len_words >
                    (USBx_HOST->HPTXSTS & 0xFFFFU)) /* split the transfer */
                {
...
HAL_StatusTypeDef USB_DoPing(USB_OTG_GlobalTypeDef *USBx, uint8_t ch_num)
{
    ...
    USBx_HC(chnum)->HCTSIZ =
        ((num_packets << 19) & USB_OTG_HCTSIZ_PKTCNT) | USB_OTG_HCTSIZ_DOPING;
