 ...
 */
HAL_StatusTypeDef
HAL_PCD_RegisterDataOutStageCallback(PCD_HandleTypeDef               *hpcd,
 ...
 */
HAL_StatusTypeDef
HAL_PCD_RegisterIsoOutIncpltCallback(PCD_HandleTypeDef               *hpcd,
...
HAL_StatusTypeDef HAL_PCD_UnRegisterIsoInIncpltCallback(PCD_HandleTypeDef *hpcd)
{
    ...
    if (hpcd->State == HAL_PCD_STATE_READY) {
        hpcd->ISOINIncompleteCallback =
            HAL_PCD_ISOINIncompleteCallback; /* Legacy weak
...
void HAL_PCD_IRQHandler(PCD_HandleTypeDef *hpcd)
{
    ...
    if (USB_GetMode(hpcd->Instance) == USB_OTG_MODE_DEVICE) {
        ...
        hpcd->FrameNumber = (USBx_DEVICE->DSTS & USB_OTG_DSTS_FNSOF_Msk) >>
        ...
        if (__HAL_PCD_GET_FLAG(hpcd, USB_OTG_GINTSTS_RXFLVL)) {
            ...
            } else if (((RegVal & USB_OTG_GRXSTSP_PKTSTS) >> 17) ==
                       STS_SETUP_UPDT) {
        ...
        if (__HAL_PCD_GET_FLAG(hpcd, USB_OTG_GINTSTS_OEPINT)) {
            ...
            while (ep_intr != 0U) {
                if ((ep_intr & 0x1U) != 0U) {
                    epint = USB_ReadDevOutEPInterrupt(hpcd->Instance,
                                                      (uint8_t)epnum);
...
#if (USE_HAL_PCD_REGISTER_CALLBACKS == 1U)
                            hpcd->ISOOUTIncompleteCallback(hpcd,
                                                           (uint8_t)epnum);
#else
                            HAL_PCD_ISOOUTIncompleteCallback(hpcd,
                                                             (uint8_t)epnum);
#endif /* USE_HAL_PCD_REGISTER_CALLBACKS */
                        }
        ...
        if (__HAL_PCD_GET_FLAG(hpcd, USB_OTG_GINTSTS_IEPINT)) {
            ...
            while (ep_intr != 0U) {
                ...
                {
                    epint = USB_ReadDevInEPInterrupt(hpcd->Instance,
                                                     (uint8_t)epnum);

                    if ((epint & USB_OTG_DIEPINT_XFRC) ==
                        USB_OTG_DIEPINT_XFRC) {
                        fifoemptymsk =
                            (uint32_t)(0x1UL << (epnum & EP_ADDR_MSK));
...
#else
                            HAL_PCD_ISOINIncompleteCallback(hpcd,
                                                            (uint8_t)epnum);
#endif /* USE_HAL_PCD_REGISTER_CALLBACKS */
                        }
        ...
        if (__HAL_PCD_GET_FLAG(hpcd, USB_OTG_GINTSTS_LPMINT)) {
            ...
            if (hpcd->LPM_State == LPM_L0) {
                ...
                hpcd->BESL =
                    (hpcd->Instance->GLPMCFG & USB_OTG_GLPMCFG_BESL) >> 2U;
        ...
        if (__HAL_PCD_GET_FLAG(hpcd, USB_OTG_GINTSTS_USBRST)) {
            ...
            if (hpcd->Init.use_dedicated_ep1 != 0U) {
                ...
                USBx_DEVICE->DOEPMSK |=
                    USB_OTG_DOEPMSK_STUPM | USB_OTG_DOEPMSK_XFRCM |
                                              ...
                                              uint32_t           epnum)
{
    ...
    while (
        ((USBx_INEP(epnum)->DTXFSTS & USB_OTG_DTXFSTS_INEPTFSAV) >= len32b) &&
        ...
        (void)USB_WritePacket(USBx, ep->xfer_buff, (uint8_t)epnum,
                              (uint16_t)len);
            ...
            if ((wIstr & USB_ISTR_DIR) == 0U) {
                ...
                if ((wEPVal & USB_EP_SETUP) != 0U) {
                    ...
                    if (((wEPVal & USB_EP_SETUP) == 0U) &&
                        ...
                        PCD_SET_EP_RX_CNT(hpcd->Instance, PCD_ENDP0,
                                          ep->maxpacket);
