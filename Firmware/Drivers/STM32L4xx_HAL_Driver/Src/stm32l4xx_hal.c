...
#define STM32L4XX_HAL_VERSION                                                  \
    ((STM32L4XX_HAL_VERSION_MAIN << 24U) |                                     \
     (STM32L4XX_HAL_VERSION_SUB1 << 16U) |                                     \
     (STM32L4XX_HAL_VERSION_SUB2 << 8U) | (STM32L4XX_HAL_VERSION_RC))
...
#define FB_MODE_BB                                                             \
    (PERIPH_BB_BASE + (MEMRMP_OFFSET * 32U) + (FB_MODE_BitNumber * 4U))

/* --- SCSR Register ---*/
/* Alias word address of SRAM2ER bit */
#define SCSR_OFFSET    (SYSCFG_OFFSET + 0x18U)
#define BRER_BitNumber 0U
#define SCSR_SRAM2ER_BB                                                        \
    (PERIPH_BB_BASE + (SCSR_OFFSET * 32U) + (BRER_BitNumber * 4U))
 ...
 * defined in user file stm32l4xx_hal_msp.c.
 *
 ...
 *         used after a system Reset and the NVIC configuration is set to
 ...
 * process, The SysTick interrupt must have higher priority (numerically lower)
 *       than the peripheral interrupt. Otherwise the caller ISR process will be
 * blocked. The function is declared as __weak  to be overwritten  in case of
 * other implementation  in user file.
 ...
 *       used to generate interrupts at regular time intervals. Once
 ...
 *         This bit is automatically reset at the end of the SRAM2 erase
 * operation.
 * @retval None
 */
