 ...
 */
#define PVM_MODE_IT                                                            \
    ((uint32_t)0x00010000) /*!< Mask for interruption yielded by PVM threshold crossing */
#define PVM_MODE_EVT                                                           \
    ((uint32_t)0x00020000) /*!< Mask for event yielded by PVM threshold crossing        */
...
HAL_StatusTypeDef HAL_PWREx_ControlVoltageScaling(uint32_t VoltageScaling)
{
    ...
    if (VoltageScaling == PWR_REGULATOR_VOLTAGE_SCALE1_BOOST) {
        ...
        if (READ_BIT(PWR->CR1, PWR_CR1_VOS) == PWR_REGULATOR_VOLTAGE_SCALE2) {
            ...
            wait_loop_index =
                ((PWR_FLAG_SETTING_DELAY_US * SystemCoreClock) / 1000000U) + 1;
 ...
 *        When VDD is present, charge the external battery on VBAT thru an
 * internal resistor.
 ...
 * @retval None
 */
void HAL_PWREx_EnableVddUSB(void) { SET_BIT(PWR->CR2, PWR_CR2_USV); }

/**
 * @brief Disable VDDUSB supply.
 * @retval None
 */
void HAL_PWREx_DisableVddUSB(void) { CLEAR_BIT(PWR->CR2, PWR_CR2_USV); }
#endif /* PWR_CR2_USV */
 ...
 * @retval None
 */
void HAL_PWREx_EnableVddIO2(void) { SET_BIT(PWR->CR2, PWR_CR2_IOSV); }

/**
 * @brief Disable VDDIO2 supply.
 * @retval None
 */
void HAL_PWREx_DisableVddIO2(void) { CLEAR_BIT(PWR->CR2, PWR_CR2_IOSV); }
#endif /* PWR_CR2_IOSV */
 ...
 *         (or PWR_GPIO_I depending on the devices) to select the GPIO
 * peripheral.
 ...
 *        PWR_PUCRx and PWR_PDCRx registers are applied in Standby and Shutdown
 * modes.
 * @note  Pull-up set by PUy bit of PWR_PUCRx register is not activated if the corresponding
 *        PDy bit of PWR_PDCRx register is also set (pull-down configuration
 * priority is higher). HAL_PWREx_EnableGPIOPullUp() and
 ...
 *        PWR_PUCRx and PWR_PDCRx registers are not applied in Standby and
 * Shutdown modes.
 * @retval None
 */
 ...
 *          This parameter can be one of the following values:
 *            @arg @ref PWR_NO_SRAM2_RETENTION        SRAM2 is powered off in
 * Standby mode (SRAM2 content is lost)
 *            @arg @ref PWR_FULL_SRAM2_RETENTION      Full SRAM2 is powered by
 * the low-power regulator in Standby mode
 *            @arg @ref PWR_4KBYTES_SRAM2_RETENTION   Only 4 Kbytes of SRAM2 is
 * powered by the low-power regulator in Standby mode
 ...
 * @retval None
 */
void HAL_PWREx_EnableBORPVD_ULP(void) { SET_BIT(PWR->CR3, PWR_CR3_ENULP); }
 ...
 * @retval None
 */
void HAL_PWREx_DisableBORPVD_ULP(void) { CLEAR_BIT(PWR->CR3, PWR_CR3_ENULP); }
#endif /* PWR_CR3_ENULP */
 ...
 *        Flash in power-down monde in setting the RUN_PD bit in FLASH_ACR
 * register. Additionally, the clock frequency must be reduced below 2 MHz.
 *        Setting RUN_PD in FLASH_ACR then appropriately reducing the clock
 * frequency must be done before calling HAL_PWREx_EnableLowPowerRunMode() API.
 * @retval None
 */
 ...
 *        REGLPF has been properly reset (otherwise,
 ...
 * RCC_CFGR register is set; the MSI oscillator is selected if STOPWUCK is
 * cleared.
 ...
 * @note  All clocks in the VCORE domain are stopped, the PLL, the MSI,
 *        the HSI and the HSE oscillators are disabled. Some peripherals with
 * wakeup capability (LCD, LPTIM1, I2C3 and LPUART) can switch on the HSI to
 * receive a frame, and switch off the HSI after receiving the frame if it is
 * not a wakeup frame. In this case the HSI clock is propagated only to the
 * peripheral requesting it. SRAM1, SRAM2 and register contents are preserved.
 *        SRAM3 content is preserved depending on RRSTP bit setting (not
 * available on all devices). The BOR is available. The voltage regulator is set
 * in low-power mode but LPR bit must be cleared to enter stop 2 mode.
 ...
 * off. SRAM1, SRAM2 and registers contents are lost except for registers in the
 * Backup domain. The BOR is not available.
