...
#define I2C_XFER_LISTEN_IT                                                     \
    (uint16_t)(                                                                \
        0x8000U) /*!< Bit field can be combinated with @ref I2C_XFER_TX_IT     \
...
static HAL_StatusTypeDef I2C_Master_ISR_IT(struct __I2C_HandleTypeDef *hi2c,
                                           uint32_t                    ITFlags,
                                           uint32_t ITSources);
static HAL_StatusTypeDef I2C_Mem_ISR_IT(struct __I2C_HandleTypeDef *hi2c,
...
static HAL_StatusTypeDef I2C_Master_ISR_DMA(struct __I2C_HandleTypeDef *hi2c,
                                            uint32_t                    ITFlags,
                                            uint32_t ITSources);
static HAL_StatusTypeDef I2C_Mem_ISR_DMA(struct __I2C_HandleTypeDef *hi2c,
                                         uint32_t ITFlags, uint32_t ITSources);
...
HAL_StatusTypeDef HAL_I2C_Init(I2C_HandleTypeDef *hi2c)
{
    ...
    if (hi2c->State == HAL_I2C_STATE_RESET) {
        ...
        hi2c->AddrCallback =
            HAL_I2C_AddrCallback; /* Legacy weak AddrCallback         */
    ...
    {
        hi2c->Instance->OAR1 =
            (I2C_OAR1_OA1EN | I2C_OAR1_OA1MODE | hi2c->Init.OwnAddress1);
    }
    ...
    /* Enable the AUTOEND by default, and enable NACK (should be disable only
     * during Slave process */
 ...
 *          @arg @ref HAL_I2C_MASTER_TX_COMPLETE_CB_ID Master Tx Transfer
 * completed callback ID
 *          @arg @ref HAL_I2C_MASTER_RX_COMPLETE_CB_ID Master Rx Transfer
 * completed callback ID
 *          @arg @ref HAL_I2C_SLAVE_TX_COMPLETE_CB_ID Slave Tx Transfer
 * completed callback ID
 *          @arg @ref HAL_I2C_SLAVE_RX_COMPLETE_CB_ID Slave Rx Transfer
 * completed callback ID
 *          @arg @ref HAL_I2C_LISTEN_COMPLETE_CB_ID Listen Complete callback ID
 *          @arg @ref HAL_I2C_MEM_TX_COMPLETE_CB_ID Memory Tx Transfer callback
 * ID
 *          @arg @ref HAL_I2C_MEM_RX_COMPLETE_CB_ID Memory Rx Transfer completed
 * callback ID
 ...
 */
HAL_StatusTypeDef
HAL_I2C_UnRegisterCallback(I2C_HandleTypeDef        *hi2c,
                           HAL_I2C_CallbackIDTypeDef CallbackID)
{
    HAL_StatusTypeDef status = HAL_OK;
 ...
 *         To be used instead of the weak HAL_I2C_AddrCallback() predefined
 * callback
 * @param  hi2c Pointer to a I2C_HandleTypeDef structure that contains
 *                the configuration information for the specified I2C.
 ...
 */
HAL_StatusTypeDef
HAL_I2C_RegisterAddrCallback(I2C_HandleTypeDef       *hi2c,
 ...
 *         Info Ready I2C Callback is redirected to the weak
    ...
    if (hi2c->State == HAL_I2C_STATE_READY) {
        ...
        if (hi2c->XferSize > 0U) {
            ...
            if (dmaxferstatus == HAL_OK) {
                ...
                /* Note : The I2C interrupts must be enabled after unlocking
                   current process
                          to avoid the risk of I2C interrupt handle execution before current
                          process unlock */
    ...
    if (hi2c->State == HAL_I2C_STATE_READY) {
        ...
        if (hi2c->XferSize > 0U) {
            if (hi2c->hdmarx != NULL) {
                ...
                /* Enable the DMA channel */
                dmaxferstatus = HAL_DMA_Start_IT(
                    hi2c->hdmarx, (uint32_t)&hi2c->Instance->RXDR,
            ...
            if (dmaxferstatus == HAL_OK) {
                /* Send Slave Address */
                /* Set NBYTES to read and reload if hi2c->XferCount >
                 * MAX_NBYTE_SIZE and generate RESTART */
                I2C_TransferConfig(hi2c, DevAddress, (uint8_t)hi2c->XferSize,
                                   xfermode, I2C_GENERATE_START_READ);
                                       ...
                                       uint16_t Size)
{
    ...
    if (hi2c->State == HAL_I2C_STATE_READY) {
        ...
        /* If Memory address size is 16Bit */
            ...
            /* Prefetch Memory Address (MSB part, LSB will be manage through
             * interrupt) */
        ...
        /* Send Slave Address and Memory Address */
        I2C_TransferConfig(hi2c, DevAddress, (uint8_t)MemAddSize,
                           I2C_RELOAD_MODE, I2C_GENERATE_START_WRITE);
                                        ...
                                        uint32_t Timeout)
{
    ...
    if (hi2c->State == HAL_I2C_STATE_READY) {
        ...
        do {
            /* Generate Start */
            hi2c->Instance->CR2 =
                I2C_GENERATE_START(hi2c->Init.AddressingMode, DevAddress);
                                                 ...
                                                 uint32_t XferOptions)
{
    ...
    if (hi2c->State == HAL_I2C_STATE_READY) {
        ...
        /* If transfer direction not change and there is no request to start
           another frame,
                                                ...
                                                uint32_t XferOptions)
{
    ...
    if (((uint32_t)hi2c->State & (uint32_t)HAL_I2C_STATE_LISTEN) ==
        ...
        /* Disable Interrupts, to prevent preemption during treatment in case of
         * multicall */
        ...
        /* I2C cannot manage full duplex exchange so disable previous IT enabled
         * if any */
            ...
            if ((hi2c->Instance->CR1 & I2C_CR1_RXDMAEN) == I2C_CR1_RXDMAEN) {
                ...
                if (hi2c->hdmarx != NULL) {
                    ...
                    if (HAL_DMA_Abort_IT(hi2c->hdmarx) != HAL_OK) {
                        /* Call Directly XferAbortCallback function in case of
                         * error */
                                          ...
                                          uint16_t           DevAddress)
{
    if (hi2c->Mode == HAL_I2C_MODE_MASTER) {
        ...
        /* Set AUTOEND mode, this will generate a NACK then STOP condition to
        ...
        /* This function should be used only in case of abort monitored by
         * master device */
        return HAL_ERROR;
    }
}
...
void HAL_I2C_ER_IRQHandler(I2C_HandleTypeDef *hi2c)
{
    ...
    if ((tmperror & (HAL_I2C_ERROR_BERR | HAL_I2C_ERROR_OVR |
                     HAL_I2C_ERROR_ARLO)) != HAL_I2C_ERROR_NONE) {
    ...
    if ((I2C_CHECK_FLAG(tmpITFlags, I2C_FLAG_AF) != RESET) &&
        ...
        *hi2c->pBuffPtr = (uint8_t)hi2c->Instance->RXDR;
        ...
        if ((hi2c->XferCount != 0U) && (hi2c->XferSize == 0U)) {
            if (hi2c->XferCount > MAX_NBYTE_SIZE) {
                hi2c->XferSize = MAX_NBYTE_SIZE;
                I2C_TransferConfig(hi2c, (uint16_t)hi2c->Devaddress,
                                   (uint8_t)hi2c->XferSize, I2C_RELOAD_MODE,
                                   I2C_NO_STARTSTOP);
            } else {
                hi2c->XferSize = hi2c->XferCount;
                                          ...
                                          uint32_t ITFlags, uint32_t ITSources)
{
    ...
    if ((I2C_CHECK_FLAG(tmpITFlags, I2C_FLAG_AF) != RESET) &&
        ...
        /* if yes, normal use case, a NACK is sent by the MASTER when Transfer
         * is finished */
        /* Mean XferCount == 0*/
        ...
        if (hi2c->XferCount == 0U) {
            ...
            /* Same action must be done for (tmpoptions == I2C_LAST_FRAME) which
               removed for
            ...
            /* if no, error use case, a Non-Acknowledge of last Data is
            ...
            if ((tmpoptions == I2C_FIRST_FRAME) ||
                (tmpoptions == I2C_NEXT_FRAME)) {
                /* Call the corresponding callback to inform upper layer of End
                 * of Transfer */
        ...
        /* If it is the case, this last write in TXDR is not sent, correspond to
         * a dummy TXIS event */
        if (hi2c->XferCount > 0U) {
            /* Write data to TXDR */
            hi2c->Instance->TXDR = *hi2c->pBuffPtr;
            ...
            if ((tmpoptions == I2C_NEXT_FRAME) ||
                (tmpoptions == I2C_FIRST_FRAME)) {
                /* Last Byte is Transmitted */
                /* Call I2C Slave Sequential complete process */
                I2C_ITSlaveSeqCplt(hi2c);
            }
    ...
    if ((I2C_CHECK_FLAG(ITFlags, I2C_FLAG_AF) != RESET) &&
        ...
        if ((I2C_CHECK_IT_SOURCE(ITSources, I2C_CR1_TXDMAEN) != RESET) ||
            ...
            if (treatdmanack == 1U) {
                if ((hi2c->State == HAL_I2C_STATE_LISTEN) &&
                    (tmpoptions == I2C_FIRST_AND_LAST_FRAME))
                /* Same action must be done for (tmpoptions == I2C_LAST_FRAME)
                   which removed for
                   Warning[Pa134]: left and right operands are identical */
                {
                    /* Call I2C Listen complete process */
                ...
                if ((tmpoptions == I2C_FIRST_FRAME) ||
                    ...
                    /* Call the corresponding callback to inform upper layer of
                     * End of Transfer */
                    I2C_ITError(hi2c, hi2c->ErrorCode);
                }
            }
...
static void I2C_ITAddrCplt(I2C_HandleTypeDef *hi2c, uint32_t ITFlags)
{
    ...
    /* In case of Listen state, need to inform upper layer of address match code
     * event */
    if (((uint32_t)hi2c->State & (uint32_t)HAL_I2C_STATE_LISTEN) ==
        (uint32_t)HAL_I2C_STATE_LISTEN) {
...
static void I2C_ITSlaveCplt(I2C_HandleTypeDef *hi2c, uint32_t ITFlags)
{
    ...
    if (hi2c->ErrorCode != HAL_I2C_ERROR_NONE) {
        ...
        /* Call the Listen Complete callback, to inform upper layer of the end
         * of Listen usecase */
        if (hi2c->State == HAL_I2C_STATE_LISTEN) {
            /* Call I2C Listen complete process */
            I2C_ITListenCplt(hi2c, tmpITFlags);
        ...
        /* Call the Sequential Complete callback, to inform upper layer of the
         * end of Transfer */
        I2C_ITSlaveSeqCplt(hi2c);
...
static void I2C_ITListenCplt(I2C_HandleTypeDef *hi2c, uint32_t ITFlags)
{
    ...
    /* Call the Listen Complete callback, to inform upper layer of the end of
     * Listen usecase */
#if (USE_HAL_I2C_REGISTER_CALLBACKS == 1)
    hi2c->ListenCpltCallback(hi2c);
#else
...
static void I2C_ITError(I2C_HandleTypeDef *hi2c, uint32_t ErrorCode)
{
    ...
    if ((hi2c->hdmatx != NULL) &&
        ((tmppreviousstate == I2C_STATE_MASTER_BUSY_TX) ||
    ...
    /* Abort DMA RX transfer if any */
             ...
             ((tmppreviousstate == I2C_STATE_MASTER_BUSY_RX) ||
        ...
        if (HAL_DMA_GetState(hi2c->hdmarx) != HAL_DMA_STATE_READY) {
            ...
            /* Abort DMA RX */
            if (HAL_DMA_Abort_IT(hi2c->hdmarx) != HAL_OK) {
                /* Call Directly hi2c->hdmarx->XferAbortCallback function in
                 * case of error */
                hi2c->hdmarx->XferAbortCallback(hi2c->hdmarx);
            }
        } else {
...
static void I2C_DMASlaveTransmitCplt(DMA_HandleTypeDef *hdma)
{
    ...
    if ((tmpoptions == I2C_NEXT_FRAME) || (tmpoptions == I2C_FIRST_FRAME)) {
        ...
        /* No specific action, Master fully manage the generation of STOP
         * condition */
        /* Mean that this generation can arrive at any time, at the end or
         * during DMA process */
        ...
        while ((__HAL_I2C_GET_FLAG(hi2c, I2C_FLAG_STOPF) == RESET) &&
            ...
            if (Timeout != HAL_MAX_DELAY) {
                if (((HAL_GetTick() - tickstart) > Timeout) ||
                    ...
                    /* In case of I2C still busy, try to regenerate a STOP
                     * manually */
 ...
 *     @arg @ref I2C_GENERATE_STOP Generate stop condition (Size should be set
 * to 0).
