...
#endif /* (USE_HAL_RNG_REGISTER_CALLBACKS) */
{
    ...
    void (*ErrorCallback)(
        struct __RNG_HandleTypeDef *hrng); /*!< RNG Error Callback         */

    void (*MspInitCallback)(
        struct __RNG_HandleTypeDef *hrng); /*!< RNG Msp Init callback      */
    void (*MspDeInitCallback)(
        struct __RNG_HandleTypeDef *hrng); /*!< RNG Msp DeInit callback    */
#endif                                     /* USE_HAL_RNG_REGISTER_CALLBACKS */
...
typedef void (*pRNG_ReadyDataCallbackTypeDef)(
    ...
    uint32_t
        random32bit); /*!< pointer to an RNG Data Ready specific callback function */
...
#define HAL_RNG_ERROR_INVALID_CALLBACK                                         \
    0x00000001U                           /*!< Invalid Callback error  */
#endif                                    /* USE_HAL_RNG_REGISTER_CALLBACKS */
...
#define __HAL_RNG_ENABLE(__HANDLE__)                                           \
    ((__HANDLE__)->Instance->CR |= RNG_CR_RNGEN)
...
#define __HAL_RNG_ENABLE_IT(__HANDLE__)                                        \
    ((__HANDLE__)->Instance->CR |= RNG_CR_IE)
...
#define IS_RNG_FLAG(FLAG)                                                      \
    (((FLAG) == RNG_FLAG_DRDY) || ((FLAG) == RNG_FLAG_CECS) ||                 \
     ((FLAG) == RNG_FLAG_SECS))
...
#define IS_RNG_CED(__MODE__)                                                   \
    (((__MODE__) == RNG_CED_ENABLE) || ((__MODE__) == RNG_CED_DISABLE))
#endif /* defined(RNG_CR_CED) */
/**
 * @}
 */
