...
#define LL_PWR_PVDLEVEL_7                                                      \
    (PWR_CR2_PLS_LEV7) /* External input analog voltage   (Compare internally  \
                          to VREFINT) */
/**
 * @}
 */
...
{
    return ((READ_BIT(PWR->CR3, PWR_CR3_DSIPDEN) == (PWR_CR3_DSIPDEN)) ? 1UL
                                                                       : 0UL);
}
...
{
    return ((READ_BIT(PWR->CR3, PWR_CR3_RRS) == (LL_PWR_FULL_SRAM2_RETENTION))
                ? 1UL
                : 0UL);
}
...
{
    return ((READ_BIT(PWR->SR2, PWR_SR2_REGLPF) == (PWR_SR2_REGLPF)) ? 1UL
                                                                     : 0UL);
}
...
{
    return ((READ_BIT(PWR->SR2, PWR_SR2_REGLPS) == (PWR_SR2_REGLPS)) ? 1UL
                                                                     : 0UL);
}
