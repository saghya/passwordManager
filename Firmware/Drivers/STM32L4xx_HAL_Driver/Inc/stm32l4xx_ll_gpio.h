...
typedef struct {
    ...
    uint32_t
        Alternate; /*!< Specifies the Peripheral to be connected to the selected pins.
...
{
    return (uint32_t)(READ_BIT(GPIOx->MODER, (GPIO_MODER_MODE0
                                              << (POSITION_VAL(Pin) * 2U))) >>
                      (POSITION_VAL(Pin) * 2U));
}
...
{
    return (uint32_t)(READ_BIT(GPIOx->AFR[0], (GPIO_AFRL_AFSEL0
                                               << (POSITION_VAL(Pin) * 4U))) >>
                      (POSITION_VAL(Pin) * 4U));
}
