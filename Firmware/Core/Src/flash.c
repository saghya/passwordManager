...
static inline uint32_t getPage(uint32_t address) { return (address - FIRST_PAGE_ADDR) / FLASH_PAGE_SIZE; }
...
uint32_t Flash_Write_Page(uint32_t Address, uint64_t *Data, uint16_t numberofdoublewords)
{
    ...
    uint32_t page_shift_after  = FLASH_PAGE_SIZE - (EndAddress % FLASH_PAGE_SIZE);
    ...
    memcpy(buff + page_shift_before + numofbytes, (uint8_t *)EndAddress, page_shift_after);

    return Flash_Write_Data(StartPageAddress, (uint64_t *)buff, (page_shift_before + numofbytes + page_shift_after) / 8);
}
...
uint32_t Flash_Write_Data(uint32_t StartPageAddress, uint64_t *Data, uint16_t numberofdoublewords)
{
    ...
    uint32_t EndPageAdress = StartPageAddress + numberofdoublewords * 8 - 8;
